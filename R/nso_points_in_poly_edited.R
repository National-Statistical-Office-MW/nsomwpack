...
nso_points <- st_read("data/hh_3857/ta_malemia_points.shp")
nso_polygon <- st_read("data/hh_3857/malemia_modi.shp")

# *****************************************************************************
nso_points_in_polygon <- function(nso_polygon, nso_points, ea_explore){
  # Changing to a common polygon
  nso_points <- st_transform(nso_points, st_crs(nso_polygon))  
  
  # Add row numbers within EA
  nso_points <- nso_points %>% 
    group_by(hh04) %>% 
    mutate(hh_no = row_number())
  ...
  distances_to_polygon <- c() # vector for distance from point to poly
  for(sel_ea in eas)
  { 
    ...
    point_distances <- st_distance(selected_ea_points, selected_ea_poly)
